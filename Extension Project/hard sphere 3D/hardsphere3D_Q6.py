    ...
    for i in range(N):
        ...
        for j in nb:
            ...
            for m in range(-l, l+1):
                qlm[i, m+l] += sph_harm_y(m, l, phi, theta)

        qlm[i] /= len(nb)

    # Local Q6 per particle
    Q6_local = np.sqrt(np.sum(np.abs(qlm)**2, axis=1))

    # Global Q6 (standard definition)
...
def mc_move(x, y, z, r, d, L):
    ...
    if step % 20000 == 0 and step > 0:
        neighbors = find_neighbors(x, y, z, L, rcut=1.4*(2*r))
        Q6_local, Q6_global = compute_Q6(x, y, z, neighbors, L)

        Q6_history.append(Q6_global)
        step_history.append(step)

        print(f"Step {step}: Q6 = {Q6_global:.4f}")
...
for density_idx, (L, color) in enumerate(zip(L_values, colors)):
    ...
    frames_dir = f"3D_frames_equil_eta_{densities[density_idx]:.2f}"
...
plt.plot(step_history, Q6_history, marker='o')
plt.xlabel("MC steps")
plt.ylabel("Global $Q_6$")
plt.title("Bond-orientational order parameter")
plt.axhline(0.57, color='r', linestyle='--', label='FCC')
plt.axhline(0.30, color='k', linestyle='--', label='Liquid')
plt.legend()
plt.tight_layout()
plt.show()
