...
def mc_move_lj(x, y, z, d, L, sigma, rcut, beta, neighbors):
...
def build_neighbor_list(x, y, z, L, r_list):
    neighbors = [[] for _ in range(len(x))]
...
def save_3d_frame_with_box(x, y, z, L, step, frames_dir, eq_frame_index, angle=None):
    ...
    for i in range (0,len(x)):
        ...
        ax.plot_surface(xs, ys, zs, cmap = 'cool')
    # Draw box edges
...
def averaged_g_r(x, y,z, r, d, L, rMax, dr, num_sims, sample, equil_steps=0, block_size=5):
    ...
    for i in range(num_sims):
        x, y, z, _ = mc_move_lj(x, y, z, d, L, 2*r, rcut, beta, neighbors)
        if i < equil_steps:
            continue
        if i % sample == 0:
